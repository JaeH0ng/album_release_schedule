...
웹 첫 화면은 `이번 주 집중 대시보드`다. 여기서 이번 주에 반드시 끝낼 것, 30분 fallback, 지연 중인 작업, 최근 완료한 작업, 주간 체크인과 코덱스에게 보낼 메시지 초안을 바로 확인할 수 있다. `공모전` 탭에서는 매주 갱신되는 싱어송라이터 공모전 카드와 `수락/보류/제외` 상태를 바로 관리할 수 있다.
