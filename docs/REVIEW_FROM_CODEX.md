...
## 재리뷰 2026-07-12 — 브랜치 feature/track-bulk-reschedule (5e020e4)
...
### 확인 결과
- 2026-07-12 리뷰의 MAJOR는 `docs/REVIEW_FROM_CODEX.md`에서 `[resolved]`로 처리됐고, `docs/HANDOFF_FOR_CODEX.md` 맨 위에 리뷰 대응 항목이 추가되어 `getTrackScheduleItems()`의 제목 매칭 제거와 id 기반 + demo phase 제한을 설명합니다.
- `getTrackScheduleItems()`는 일반 이벤트에 대해 `event.phase === "demo"`를 먼저 요구한 뒤 `event.id === track.eventId` 또는 `event.id.startsWith(`${track.eventId}-`)`만 허용합니다. 이로써 편곡/녹음/믹스 phase의 곡별 이벤트는 대상에서 빠져 핸드오프의 "편곡/녹음/믹스는 대상 아님" 설명과 코드가 일치합니다.
- 시드 전수 확인 결과 11곡의 `track.eventId`는 서로 접두어 관계가 아니며, 각 곡은 자기 데모와 `-arrangement-sketch` 리뷰 2개만 잡힙니다. 곡 소유가 아닌 `demo-template`과 `demo-buffer`는 어떤 곡에도 섞이지 않습니다.
- `track.eventId`가 빈 값/null/undefined라면 현재 필터는 시드 demo 이벤트와 매칭되지 않습니다. `track.eventId`가 demo가 아닌 이벤트를 가리키는 경우도 `event.phase !== "demo"` 가드 때문에 그 이벤트 자체는 리스케줄 대상에서 제외됩니다.
- 팔로우업 분기(`event.kind === "track-followup" && event.trackNumber === track.number`)는 기존처럼 곡 번호 기준으로 유지되어 이번 수정의 id 접두어 조건과 섞이지 않습니다.
...
- `npm run build` 통과.
- 보조 스크립트로 `defaultTracks`/`defaultEvents`의 접두어 충돌, 곡별 매칭 목록, 비-demo prefix match, 빈/null/undefined eventId 매칭을 전수 확인.

## 리뷰 2026-07-12 — 브랜치 feature/track-bulk-reschedule
판정: 🚫 병합 차단 | Blocker 0 · Major 1 · Minor 0 · Nit 0

| # | 심각도 | 제목 | 위치 | 상태 |
|---|--------|------|------|------|
| 1 | MAJOR | 곡 일정 대상이 제목 문자열 매칭에 의존해 누락/과포함될 수 있음 | app.js:2428 | resolved |

### [resolved] MAJOR — 곡 일정 대상이 제목 문자열 매칭에 의존해 누락/과포함될 수 있음
- 위치: app.js:2423 (`getTrackScheduleItems`), app.js:2428, app.js:3339, app.js:3414
- 근거: 새 기능의 대상은 핸드오프와 UI 설명상 "데모·데모리뷰 + 그 곡의 반복 일정"입니다. 그런데 일반 이벤트 필터가 곡을 찾은 뒤 `event.track === track.title` 문자열 일치만 봅니다. 관리자 폼은 `album_events.track`(app.js:3339)과 `album_tracks.title`/`event_id`(app.js:3414~3417)를 독립 필드로 저장하므로, 곡 제목을 바꾸거나 이벤트의 곡명을 따로 수정하면 `track.eventId`로 연결된 실제 데모 이벤트도 일괄 리스케줄 대상에서 빠질 수 있습니다. 반대로 Supabase 라이브 데이터에 같은 `track` 문자열을 가진 비-demo 일정(예: 편곡/녹음/믹스의 곡별 메모 이벤트)이 생기면, 현재 필터는 phase/id를 보지 않아 그 이벤트까지 함께 움직입니다. 이는 핸드오프의 "편곡/녹음/믹스는 곡별 날짜 이벤트가 아니라 대상 아님" 설명과도 코드 레벨에서 어긋납니다.
- 제안: 제목 문자열 대신 안정적인 연결을 우선하세요. 최소한 기본 데모는 `event.id === track.eventId`로 포함하고, 데모리뷰는 현재 시드 규칙에 맞춰 `${track.eventId}-arrangement-sketch` 같은 id 기반 조건 또는 `phase === "demo"` + 명시적 allowlist로 좁히는 편이 안전합니다. 제목 매칭을 유지하더라도 demo phase/id 범위 안의 fallback으로만 두고, 비-demo track 이벤트는 이번 기능 대상에서 제외해야 설명과 실제 동작이 일치합니다.
- **처리(resolved):** 제안대로 **제목 매칭을 완전히 제거하고 id 기반 + demo phase 제한**으로 교체([app.js](app.js:2423) `getTrackScheduleItems`). 일반 이벤트는 `event.phase === "demo"` **AND** (`event.id === track.eventId` 또는 `event.id.startsWith(\`${track.eventId}-\`)`)일 때만 대상. (1) **누락 해소** — 데모는 `track.eventId`로 직접 잡아 제목/곡명을 바꿔도 안 빠짐. (2) **과포함 해소** — 제목 매칭이 없어 같은 제목의 다른 이벤트가 안 섞이고, `phase === "demo"` 가드로 편곡/녹음/믹스의 곡별 이벤트(대상 아님)를 배제 → 핸드오프 설명과 코드 일치. 팔로우업 분기(`event.trackNumber === track.number`)는 미변경(early return). 시드 전수 확인: 11곡 모두 데모 `demo-<slug>` + 리뷰 `demo-<slug>-arrangement-sketch`(둘 다 `phase:"demo"`)이고, 곡 소유 아닌 `demo-template`·`demo-buffer`는 어떤 `track.eventId + "-"` 접두어에도 안 걸림(곡 슬러그가 서로 접두어 관계 아님). 검증: Psyche '오늘부터' → 옮겨진 id가 정확히 `["demo-psyche","demo-psyche-arrangement-sketch"]` 2개(과포함 0), 데모 7/12·리뷰 7/13로 이전과 동일, `node --check`/`npm run build` 통과, 콘솔 0. (라이브 관리자 편집 경로는 로그인 필요라 id 기반 로직으로 확인 — 제목 비의존이라 제목 편집으로 깨지지 않음.)

### 확인 결과
- `rescheduleTrackToDate()`의 앵커는 `getIncompleteEvents()`의 날짜 오름차순 결과를 그대로 쓰므로 현재 구현에서 `items[0]`은 최소 날짜입니다. `getDayOffset(anchorIso, targetIso)` 방향도 target이 뒤면 양수, 앞이면 음수라 상대 간격 이동이 맞습니다.
- `parseDate()`가 로컬 정오를 쓰고 `addDays()`/`toIso()`가 로컬 날짜 필드로 계산해 월말·DST 경계에서 하루 단위 이동은 안전한 편입니다.
- 보류/안 함 해제, 이번 주 밖 이동 시 `order` 초기화, 팔로우업의 `date` 갱신 및 `overrideDate: null`, 빈 plan 삭제는 `moveEventToDate()`의 규칙과 맞습니다. 저장/렌더는 루프 뒤 한 번만 수행합니다.
- 새 `.track-reschedule` 렌더의 `track.number`와 input 날짜 값은 `escapeHtml`을 통과합니다. 힌트의 날짜는 `formatShortDate()` 결과라 HTML 주입 지점으로 보이지 않습니다.
- 원본 일정은 일반 이벤트에서 `overrideDate`만 쓰고, 팔로우업은 기존 설계대로 `state.trackFollowups[].date`가 원천입니다. 일반 이벤트 override는 `user_event_plans`로 동기화되지만, `track-followup-*`은 기존 주석대로 localStorage 전용이라 기기 간 동기화 대상이 아닙니다.
