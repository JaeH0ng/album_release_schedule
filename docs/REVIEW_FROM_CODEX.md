...
## 리뷰 2026-07-06 — 브랜치 claude/goofy-antonelli-0cce62
...
### 확인 결과
- `#track-chip-nav`는 `role="group"`과 `aria-label="곡 선택"`을 유지하고 있어, 곡 선택 버튼 묶음의 접근성 이름이 유지됩니다.
...
- `track-chip` 렌더 경로에서 `role="tab"`/`aria-selected`가 제거된 것을 확인했습니다. 검색에 남는 `role="tab"`/`aria-selected`는 `index.html`의 상단 뷰 전환 탭이며, `aria-controls`가 있는 별도 UI라 이번 변경 범위와 충돌하지 않습니다.
- 시각 상태는 기존 `.is-active`/`.is-done` 클래스에 그대로 의존하므로 이번 ARIA 속성 교체로 CSS 회귀는 보이지 않습니다.
